...
## ⚠️ OUTPUT CONTRACT (OBLIGATORIO)

Antes de escribir cualquier archivo, leé **[CONTRACT.md](../../../CONTRACT.md)** (raíz del repo). Reglas críticas que esta skill DEBE respetar:

1. **Todo entregable va a `outputs/<flow>/<run_slug>/`** — nunca en la raíz, en `runs/`, ni en `brands/**/`.
2. **`run_slug` = `<descripcion-kebab>-<YYYYMMDD-HHMMSS>`** (minúsculas, guiones, timestamp al final).
3. **Probes/experimentos → `scratch/`; intermedios de un run → `<run>/tmp/`.** Nunca en `outputs/` top-level.

Mapa de carpetas por flow en CONTRACT.md. Antes de cerrar el run, corré `bash scripts/validate-outputs.sh` y corregí toda violación.
...
### HyperFrames B-roll default

When the task is B-roll, avatar top-band video, social reel support visuals, or any HyperFrames composition meant to accompany spoken content, default to **iOS26 Liquid Glass / HTML-in-Canvas** unless the user or an existing brand system explicitly asks for another style.

- Official style reference: `https://hyperframes.heygen.com/catalog/blocks/ios26-liquid-glass`
- Registry install: run `npx hyperframes@latest add html-in-canvas --no-clipboard` inside the composition project before building. This installs `ios26-liquid-glass` plus the companion HTML-in-Canvas blocks.
- Live Chrome preview requires `chrome://flags/#canvas-draw-element`; CLI render should still be validated through HyperFrames.
- Visual language: dark iOS wallpaper depth, translucent/refraction glass panels, notifications, widgets, dock/app metaphors, specular highlights, blur, emoji motifs, micro-motion, and layered foreground/background parallax.
- Rhythm rule: no more than 10 seconds without a visible scene, layout, object, text, camera, or motion-state change. For long VSL/support videos, aim for 4-7 second beats and keep copy short.
- QA rule: run HyperFrames `inspect` and `lint`, render a contact sheet or frame samples, and use video perception or an equivalent visual review before delivery.
