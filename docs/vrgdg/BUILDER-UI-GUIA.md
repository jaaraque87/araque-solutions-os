...
**⚠ ACTUALIZACIÓN 2026-07-09: el update de pack NO arregló el crash** — el código de `notify` es idéntico en d6dde1fd y 4cfc788 (el guard revisa que exista la clave "workflow" pero no que su valor no sea null). **CAUSA RAÍZ REAL: el workspace de ComfyDeploy (iframe) no serializa el grafo → manda `extra_pnginfo.workflow = null`**; el creador diseña para ComfyUI vanilla. **WORKAROUND: abrir el ComfyUI de la sesión por su TUNNEL URL directa** (GET /api/sessions → tunnel_url) → agregar el nodo Builder → Load Project → Render All. Regla general: **el Builder SIEMPRE por tunnel URL, nunca por el workspace de ComfyDeploy.**
