...
- ⚠ **PRODUCTO EN MANO: CERO acciones de manipulación.** Cada verbo sobre el objeto (tap, tilt, lower, raise) multiplica el riesgo de que LTX lo suelte, lo rote o haga malabares (QA piloto 002, SEG 3). Describir mano y objeto como completamente estáticos (`perfectly still, pressed against her chest, motionless during the entire shot`) y poner TODA la actuación en la cara (cejas, sonrisa, nod). Negativos: `no hand movement, no dropping the can, no can rotation, no can morphing, no deformed can, no melted can`.
- ⚠ **AUDITORÍA DE MANOS en selfie: solo hay UNA mano libre.** Una mano sostiene el teléfono; si la otra sostiene el producto, NO puede además señalar/gesticular — pedirle un gesto a esa mano obliga a LTX a soltar el objeto y lo deja flotando (QA piloto 002, SEG 4 CTA). Antes de entregar un prompt: contar las manos y qué hace cada una en cada oración.
- ⚠ **DOSIS DE QUIETUD: máximo UNA oración de "motionless/static" (solo mano+objeto) y SIEMPRE contrapesada con articulación explícita** (`She is talking directly to the camera... Her mouth articulates every word clearly, lips moving in sync with the audio throughout the entire clip`). Tres órdenes de quietud en un prompt mataron el lipsync completo (QA piloto 002, SEG 4).
- ⚠ **TEXTO EN PRODUCTO solo sobrevive en b-roll estático.** En clips de avatar hablando, las letras se deforman con el mínimo movimiento. Regla: la legibilidad del label la carga el hero b-roll; en el CTA el producto sale DE CUADRO (first frame = recorte cerrado cara+hombros de la misma imagen, manteniendo 9:16 y ≥576×1024) — sin objeto no hay letras que cuidar y el lipsync mejora.
